...
static void _OnClerkSelectChangeFnc(CWnd *pWnd, int nOldItemSel, int nNewItemSel){
	((CFMDischargeDepositUnpaidReport* )pWnd)->OnClerkSelectChange(nOldItemSel, nNewItemSel);
} 
static void _OnClerkSelendokFnc(CWnd *pWnd){
	((CFMDischargeDepositUnpaidReport *)pWnd)->OnClerkSelendok();
}
/*static void _OnClerkSetfocusFnc(CWnd *pWnd){
	((CFMDischargeDepositUnpaidReport *)pWnd)->OnClerkKillfocus();
}*/
/*static void _OnClerkKillfocusFnc(CWnd *pWnd){
	((CFMDischargeDepositUnpaidReport *)pWnd)->OnClerkKillfocus();
}*/
static long _OnClerkLoadDataFnc(CWnd *pWnd){
	return ((CFMDischargeDepositUnpaidReport *)pWnd)->OnClerkLoadData();
}
/*static void _OnClerkAddNewFnc(CWnd *pWnd){
	((CFMDischargeDepositUnpaidReport *)pWnd)->OnClerkAddNew();
}*/
...
static int _OnAddFMDischargeDepositUnpaidReportFnc(CWnd *pWnd){
	 return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnAddFMDischargeDepositUnpaidReport();
} 
static int _OnEditFMDischargeDepositUnpaidReportFnc(CWnd *pWnd){
	 return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnEditFMDischargeDepositUnpaidReport();
} 
static int _OnDeleteFMDischargeDepositUnpaidReportFnc(CWnd *pWnd){
	 return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnDeleteFMDischargeDepositUnpaidReport();
} 
static int _OnSaveFMDischargeDepositUnpaidReportFnc(CWnd *pWnd){
	 return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnSaveFMDischargeDepositUnpaidReport();
} 
static int _OnCancelFMDischargeDepositUnpaidReportFnc(CWnd *pWnd){
	 return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnCancelFMDischargeDepositUnpaidReport();
}

static int _OnObjectListCheckAllFnc(CWnd *pWnd){
	return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnObjectListCheckAll();
}
static int _OnObjectListUnCheckAllFnc(CWnd *pWnd){
	return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnObjectListUnCheckAll();
}
static int _OnDeptListCheckAllFnc(CWnd *pWnd){
	return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnDeptListCheckAll();
}
static int _OnDeptListUnCheckAllFnc(CWnd *pWnd){
	return ((CFMDischargeDepositUnpaidReport*)pWnd)->OnDeptListUnCheckAll();
}

CFMDischargeDepositUnpaidReport::CFMDischargeDepositUnpaidReport(CWnd* pParent)
{
	m_nDlgWidth = 450;
	m_nDlgHeight = 555;
	SetDefaultValues();
...
void CFMDischargeDepositUnpaidReport::GetDataToScreen(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CFMDischargeDepositUnpaidReport::GetScreenToData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMDischargeDepositUnpaidReport::OnClerkSelectChange(int nOldItemSel, int nNewItemSel){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
} 
void CFMDischargeDepositUnpaidReport::OnClerkSelendok(){
	 
}
/*void CFMDischargeDepositUnpaidReport::OnClerkSetfocus(){
	
}*/
/*void CFMDischargeDepositUnpaidReport::OnClerkKillfocus(){
	
}*/
long CFMDischargeDepositUnpaidReport::OnClerkLoadData()
{
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
void CFMDischargeDepositUnpaidReport::OnExportSelect()
{
	...
	xls.SetCellText(nCol + 4, nRow + 5, _T("Ti\x1EC1n t\x1EA1m g\x1EEDi"), FMT_TEXT | FMT_CENTER | FMT_VCENTER | FMT_WRAPING, true, 11);
	...
	xls.SetCellText(nCol + 6, nRow + 5, _T("Ti\x1EC1n \x63hi ph\xED \x111\x1B0\x1EE3\x63 gi\x1EA3m"), FMT_TEXT | FMT_CENTER | FMT_VCENTER | FMT_WRAPING, true, 11);
	...
	xls.SetCellText(nCol + 8, nRow + 5, _T("Thu"), FMT_TEXT | FMT_CENTER | FMT_VCENTER | FMT_WRAPING, true, 11);

	xls.SetCellText(nCol + 9, nRow + 5, _T("\x43hi"), FMT_TEXT | FMT_CENTER | FMT_VCENTER | FMT_WRAPING, true, 11);
...
int CFMDischargeDepositUnpaidReport::OnAddFMDischargeDepositUnpaidReport(){
...
int CFMDischargeDepositUnpaidReport::OnEditFMDischargeDepositUnpaidReport(){
 	if(GetMode() != VM_VIEW)
 		return -1;
 	CHMSMainFrame *pMF = (CHMSMainFrame *) AfxGetMainWnd();
 	SetMode(VM_EDIT);
...
int CFMDischargeDepositUnpaidReport::OnDeleteFMDischargeDepositUnpaidReport(){
 	if(GetMode() != VM_VIEW)
 		return -1;
 	CHMSMainFrame *pMF = (CHMSMainFrame *)AfxGetMainWnd();
 	CString szSQL;
...
int CFMDischargeDepositUnpaidReport::OnSaveFMDischargeDepositUnpaidReport(){
 	if(GetMode() != VM_ADD && GetMode() != VM_EDIT)
 		return -1;
 	if(!IsValidateData())
 		return -1;
...
int CFMDischargeDepositUnpaidReport::OnCancelFMDischargeDepositUnpaidReport(){
 	if(GetMode() == VM_EDIT)
 	{
 		SetMode(VM_VIEW);
 	} 
...
int CFMDischargeDepositUnpaidReport::OnFMDischargeDepositUnpaidReportListLoadData(){
	return 0;
}

int CFMDischargeDepositUnpaidReport::OnDeptListCheckAll()
{
...
int CFMDischargeDepositUnpaidReport::OnDeptListUnCheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (m_wndDeptList.GetCheck(i))
...
int CFMDischargeDepositUnpaidReport::OnObjectListCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (!m_wndObjectList.GetCheck(i))
...
int CFMDischargeDepositUnpaidReport::OnObjectListUnCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (m_wndObjectList.GetCheck(i))
...
CString CFMDischargeDepositUnpaidReport::GetQueryString(){
	...
	szSQL.Format(_T(" SELECT    Get_patientname(docno)     pname, ") \
				_T("           Get_departmentname(deptid) dept, ") \
				_T("           recordno, ") \
				_T("           deposit, ") \
				_T("           feeamt, ") \
				...
				_T("					  hfe_amount,") \
				_T("					  hcr_dischargedate as hfe_date") \
				...
				_T("			WHERE hfe_date < Cast_string2timestamp('%s')") \
				_T("            GROUP     BY docno, ") \
				_T("                         deptid, ") \
				_T("                         recordno, ") \
				_T("                         refidx) ") \
