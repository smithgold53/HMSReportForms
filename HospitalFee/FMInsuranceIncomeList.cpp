...
static long _OnObjectLoadDataFnc(CWnd *pWnd){
	return ((CFMInsuranceIncomeList *)pWnd)->OnObjectLoadData();
}
...
static long _OnDeptListLoadDataFnc(CWnd *pWnd){
	return ((CFMInsuranceIncomeList*)pWnd)->OnDeptListLoadData();
} 
static void _OnDeptListDblClickFnc(CWnd *pWnd){
	((CFMInsuranceIncomeList*)pWnd)->OnDeptListDblClick();
} 
static void _OnDeptListSelectChangeFnc(CWnd *pWnd, int nOldItem, int nNewItem){
	((CFMInsuranceIncomeList*)pWnd)->OnDeptListSelectChange(nOldItem, nNewItem);
} 
static int _OnDeptListDeleteItemFnc(CWnd *pWnd){
	 return ((CFMInsuranceIncomeList*)pWnd)->OnDeptListDeleteItem();
} 
static int _OnDeptListCheckAllFnc(CWnd *pWnd){
	return ((CFMInsuranceIncomeList*)pWnd)->OnDeptListCheckAll();
}
static int _OnDeptListUncheckAllFnc(CWnd *pWnd){
	return ((CFMInsuranceIncomeList*)pWnd)->OnDeptListUncheckAll();
}
static void _OnLockedSelectFnc(CWnd *pWnd){
	 ((CFMInsuranceIncomeList*)pWnd)->OnLockedSelect();
}
...
void CFMInsuranceIncomeList::OnCreateComponents(){
	...
	m_wndDeptInfo.Create(this, _T("Dept"), 10, 320, 425, 545);
	m_wndReceiptSheet.Create(this, _T("Receipt Sheet"), 10, 120, 425, 315);
	...
	m_wndList.Create(this,15, 145, 420, 310); 
	m_wndDeptList.Create(this,15, 345, 420, 540); 
...
long CFMInsuranceIncomeList::OnObjectLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
int CFMInsuranceIncomeList::OnListLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	UpdateData(true);
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere;
...
void CFMInsuranceIncomeList::OnDeptListDblClick(){
	
}
 
void CFMInsuranceIncomeList::OnDeptListSelectChange(int nOldItem, int nNewItem){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	
}
 
int CFMInsuranceIncomeList::OnDeptListDeleteItem(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	 return 0;
}
 
long CFMInsuranceIncomeList::OnDeptListLoadData(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CFMInsuranceIncomeList::OnLockedSelect(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
CString CFMInsuranceIncomeList::GetQueryString(){
	...
	else
	{
		szWhere.Format(_T(" AND i.hfe_objectid <> 7 AND i.hfe_cash_id > 0 AND i.hfe_status = 'P' ") \
					_T(" AND i.hfe_date BETWEEN cast_string2timestamp('%s') AND cast_string2timestamp('%s')"), m_szFromDate, m_szToDate);
		if (!szDepts.IsEmpty())
	...
	szSQL.Format(_T(" SELECT    Get_patientname(doc_no)                                                      patient_name, ") \
				...
				_T("           CEIL(SUM(pol_amt))                                                                 pol_amt, ") \
				_T("           CEIL(SUM(ins_amt))                                                                 ins_amt, ") \
				_T("           CEIL(SUM(other_amt))                                                               other_amt, ") \
				_T("           CEIL(SUM(deposit))                                                                 deposit, ") \
				_T("           CEIL(SUM(deposit + ins_amt + pol_amt + other_amt + food_fee - deposit_extraction)) total_income ") \
				...
				_T("                             AND i.hfe_objectid IN ( 1, 3, 8 ) ) THEN f.hfe_patpaid ELSE 0 END pol_amt, ") \
				_T("                      CASE WHEN ( f.hfe_type NOT IN ('F') ") \
				_T("                             AND i.hfe_objectid NOT IN ( 1, 3, 7, 8 ) ) THEN f.hfe_patpaid ELSE 0 END ins_amt, ") \
				_T("                      0   other_amt, ") \
				_T("                      0   deposit ") \
				_T("            FROM      hms_fee_invoice i ") \
...
int CFMInsuranceIncomeList::OnDeptListCheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (!m_wndDeptList.GetCheck(i))
...
int CFMInsuranceIncomeList::OnDeptListUncheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (m_wndDeptList.GetCheck(i))
