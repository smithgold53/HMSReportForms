...
CFMTempMoneyReport::CFMTempMoneyReport(CWnd *pParent)
	...
	m_nDlgWidth = 445;
	m_nDlgHeight = 215;
	m_szTitle = _T("\x44\x61nh s\xE1\x63h \x42N t\x1EA1m g\x1EEDi ti\x1EC1n vi\x1EC7n ph\xED");
	SetDefaultValues();
}
...
CString CFMTempMoneyReport::GetQueryString(){
	...
	szSQL.Format(_T(" SELECT 	trim(hp_surname||' '||hp_midname||' '||hp_firstname) AS Name, ") \
					...
					_T(" AND hfi_recvdate BETWEEN cast_string2timestamp('%s') AND cast_string2timestamp('%s') %s") \
