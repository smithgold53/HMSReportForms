...
static int _OnObjectListCheckAllFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*)pWnd)->OnObjectListCheckAll();
}
static int _OnObjectListUnCheckAllFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*)pWnd)->OnObjectListUnCheckAll();
}
static int _OnDeptListCheckAllFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*)pWnd)->OnDeptListCheckAll();
}
static int _OnDeptListUnCheckAllFnc(CWnd *pWnd){
	return ((CFMUndischargedAdvancePatientList*)pWnd)->OnDeptListUnCheckAll();
}
...
void CFMUndischargedAdvancePatientList::OnPrintFormat1(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMUndischargedAdvancePatientList::OnPrintFormat2(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
...
void CFMUndischargedAdvancePatientList::OnExportSelect(){
	...
	menu.CreatePopupMenu();
	TranslateString(_T("Export Format 1"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 1, tmpStr);
	menu.AppendMenu(MF_SEPARATOR);
	TranslateString(_T("Export Format 2"), tmpStr);
	menu.AppendMenu(MF_BYPOSITION, 2, tmpStr);
	CRect rect;
	m_wndPrint.GetWindowRect(&rect);
	int nPos = menu.TrackPopupMenu(TPM_RETURNCMD|TPM_RIGHTALIGN|TPM_BOTTOMALIGN|TPM_NONOTIFY, rect.right, rect.top, this);
...
void CFMUndischargedAdvancePatientList::OnExportFormat1(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
...
void CFMUndischargedAdvancePatientList::OnExportFormat2(){
	CHMSMainFrame *pMF = (CHMSMainFrame*) AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL;
	CString tmpStr, szTemp;
...
CString CFMUndischargedAdvancePatientList::GetQueryString(){
	...
	szSQL.Format(_T(" SELECT   Get_patientname(hfe_docno) patient_name, ") \
				...
				_T(" WHERE     hfe_date BETWEEN Cast_string2timestamp('%s') AND Cast_string2timestamp('%s') %s") \
				_T("       AND hcr_status <> 'T' ") \
				_T("       AND hfe_status = 'P' ") \
				_T(" ORDER     BY hfe_deptid, hfe_docno "), m_szFromDate, m_szToDate, szWhere);

	return szSQL;
}

CString CFMUndischargedAdvancePatientList::GetQueryString1(){
		CString szSQL, szWhere, szObjects, szDepts;
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (m_wndObjectList.GetCheck(i))
...
int CFMUndischargedAdvancePatientList::OnDeptListCheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (!m_wndDeptList.GetCheck(i))
...
int CFMUndischargedAdvancePatientList::OnDeptListUnCheckAll()
{
	for (int i = 0; i < m_wndDeptList.GetItemCount(); i++)
	{
		if (m_wndDeptList.GetCheck(i))
...
int CFMUndischargedAdvancePatientList::OnObjectListCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (!m_wndObjectList.GetCheck(i))
...
int CFMUndischargedAdvancePatientList::OnObjectListUnCheckAll()
{
	for (int i = 0; i < m_wndObjectList.GetItemCount(); i++)
	{
		if (m_wndObjectList.GetCheck(i))
