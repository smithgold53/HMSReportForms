...
static int _OnCardListDeleteItemFnc(CWnd *pWnd){
	 return ((CFMOutpatientInsuranceCost80A*)pWnd)->OnCardListDeleteItem();
} 
...
static int _OnDeptListDeleteItemFnc(CWnd *pWnd){
	 return ((CFMOutpatientInsuranceCost80A*)pWnd)->OnDeptListDeleteItem();
} 
...
void CFMOutpatientInsuranceCost80A::OnSetWindowEvents(){
	...
	m_wndCardList.AddEvent(1, _T("Delete"), _OnCardListDeleteItemFnc, 0, VK_DELETE, 0);
...
void CFMOutpatientInsuranceCost80A::OnPrintSelect(){
	...
	if (!rpt.Init(_T("Reports/MS/F_BANGKETHANHTOAN_MAU80A.RPT")))
		return;
...
CString CFMOutpatientInsuranceCost80A::GetQueryString(){
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere, szSubWhere, szOrderBy, szTable, szGroup;
	...
	if (!szCards.IsEmpty())
	{
		szSubWhere.AppendFormat(_T(" AND substr(hc_cardno,1,2) IN(%s) "), szCards);
		//MessageBox(szCards);
	}
	...
	if (m_bByDischargedDate)
		...
		szWhere.AppendFormat(_T(" AND hcr_dischargedate BETWEEN cast_string2timestamp('%s') AND cast_string2timestamp('%s') ") \
							_T("  AND NVL(hcrf_acceptedfee, 'N') = 'N'"), m_szFromDate, m_szToDate);
	else
	...
	szSQL.Format(_T(" SELECT hd_docno AS docno,") \
				...
				_T(" WHERE ho_type IN('I','C') AND LENGTH(hc_cardno) > 1 %s") \
				_T(" GROUP BY hd_docno, hp_surname, hp_midname, hp_firstname, hp_birthdate, hp_sex, hc_cardno, hd_insline, ") \
				...
				_T(" ORDER BY %s"), szMaterialID, szMaterialID, szMaterialID, szGroup, szWhere, szWhere, szSubWhere, szOrderBy);
	return szSQL;
}

CString CFMOutpatientInsuranceCost80A::GetQueryString1(){
	CHMSMainFrame *pMF = (CHMSMainFrame*)AfxGetMainWnd();
	CRecord rs(&pMF->m_db);
	CString szSQL, szWhere, szSubWhere, szOrderBy, szTable, szGroup;
	CString szDepts, szCards;
	...
	szSQL.Format(_T(" SELECT hd_docno AS docno,") \
				...
				_T("   LEFT JOIN hms_treatment_record ON (hcr_docno = htr_docno AND hcr_refidx = htr_idx)") \
				_T("   LEFT JOIN hms_fee_invoice fi ON (hcr_docno = fi.hfe_docno AND hcrf_invoicefee = fi.hfe_invoiceno)") \
				_T("   LEFT JOIN hms_fee_view fe ON(fe.hfe_docno=hcr_docno AND fe.hfe_invoiceno = fi.hfe_invoiceno)") \
				_T("   LEFT JOIN hms_fee_list ON (fe.hfe_itemid = hfl_feeid)") \
				_T("   WHERE fe.hfe_discount > 0 AND fe.hfe_category <> 'Y' AND hcr_status='T'") \
				_T("         AND fe.hfe_type <> 'V' %s") \
				_T("   UNION ALL") \
				_T("   SELECT fi.hfe_deptid as deptid,") \
				_T("         fi.hfe_class,") \
				...
				_T("   FROM hms_clinical_record ") \
				_T("   LEFT JOIN hms_treatment_record ON (hcr_docno = htr_docno AND hcr_refidx = htr_idx)") \
				_T("   LEFT JOIN hms_fee_invoice fi ON (hcr_docno = fi.hfe_docno AND hcrf_invoiceno = fi.hfe_invoiceno)") \
				_T("   LEFT JOIN hms_fee_view fe ON(fe.hfe_invoiceno=fi.hfe_invoiceno AND fe.hfe_docno=fi.hfe_docno)") \
				_T("   WHERE fi.hfe_class IN('A', 'I') AND fe.hfe_discount > 0 AND fe.hfe_category <> 'Y' AND hcr_status='T'") \
				_T("         AND fe.hfe_type = 'V' %s") \
				_T(" ) tbl") \
